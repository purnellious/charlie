...
| `context-archive.md` | Distilled topic archives — loaded into Charlie's live system prompt in full every turn | Indefinite; append-only, but fully-resolved/superseded entries get manually moved out to `context-archive-cold.md` (see below) rather than accumulating forever, since every entry here costs tokens on every conversation |
| `context-archive-cold.md` | Archived entries moved out of `context-archive.md` once resolved/superseded — reference only | Indefinite; not loaded into Charlie's system prompt or the morning briefing |
...
| `data/focus.json` | Single current "priority sequencing" note anchoring the morning briefing's "Today's focus" section (added 2026-09-17) — `{note, expires_on, set_on}`, written conversationally via `set_current_focus` (no approval gate, same profile as `add_reminder`) | Self-expiring by design: `set_current_focus` requires a future `expires_on` and `get_active_focus()` treats a past-expiry record as absent — deliberately built this way so a stale sequencing note can't be parroted back indefinitely the way a hand-written relative day-count in `charlie.md` was (see devlog 2026-09-17). Overwritten by each new `set_current_focus` call — no history kept. |
...
*Last updated: 2026-09-17 (added `data/focus.json` for the new `set_current_focus` tool, and split `context-archive.md` into a hot/cold pair — `context-archive-cold.md` now holds resolved/superseded entries so they stop costing tokens in Charlie's live system prompt)*
