...
**Write capability (live): archive / mark read / mark unread only, on-request only.** `archive_email`/`mark_email_read`/`mark_email_unread` act on Jonathan's own mailbox (Gmail's `threads.modify`), execute immediately with no confirmation gate — reversible, self-mailbox, and only ever triggered by Jonathan explicitly asking in conversation, never by the background pipeline. **No send, reply, forward, or delete capability exists in code anywhere in this codebase.** Important nuance: the `gmail.modify` scope required for archive/mark-read is *also* technically sufficient for the Gmail API's send and reversible-trash operations (there is no narrower scope that supports label changes) — so the credential itself is capable of more than the code exposes. The boundary from here on is deliberately the tool code, not the OAuth grant; see [[BUG-018]]. **Standing rule, permanent:** the full `https://mail.google.com/` scope must never be requested for this integration — that is what keeps permanent, unrecoverable deletion architecturally impossible (Gmail's `messages.delete` requires that scope specifically; `messages.trash`, which is reversible, only needs the already-granted `gmail.modify`). Any future build proposing to request that scope needs an explicit, separate conversation with Jonathan, not a routine Tier-3 sign-off.
...
*Last updated: 2026-07-30 (added archive_email/mark_email_read/mark_email_unread tools — first write capability, on-request only, no confirmation gate; OAuth scope changed gmail.readonly → gmail.modify; added the permanent never-request-mail.google.com rule)*
