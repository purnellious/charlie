...
**Deliberately left open:** per this log's own standard (see BUG-030), a local test against mocked/real data isn't the same as proof it works live. Needs deployment to the always-on Mac and a real long-thread read through Charlie in Telegram, confirmed against the Gmail API.
