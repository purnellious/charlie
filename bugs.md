...
**Second fix, 2026-08-14 — real code-level enforcement added, per Jonathan's follow-up ask.** After a conversation with Jonathan about *when* a genuine hook (not just a prompt nudge) can and can't apply — a hook can only intercept at a real checkpoint in the code (an incoming message, a tool call, a tool result), never mid-generation inside the model's own reasoning — a real one turned out to fit the specific "reused stale context" half of this bug (not the "insufficiently broad negative" half, which stays prompt-level): `_looks_like_email_state_question()` regex-detects an email-state question ("was X forwarded", "did I ever get...", "does X exist") in Jonathan's incoming message, and when it matches, forces the turn's *first* API call to pass a `tools` list restricted to just `search_email`/`read_email_thread` plus `tool_choice: {"type": "any"}` — an API-level structural guarantee (confirmed compatible with thinking enabled on the plain Claude API, unlike Amazon Bedrock which requires disabling thinking for a forced tool_choice) that Charlie cannot reply with plain text off stale context on that call; it must make a real lookup first. Detection is a heuristic, verified against 8 positive and 9 negative phrasings (two initial misses — bare "send"/"get" verb forms — fixed before landing; "get"/"got" scoped to only count when paired with an email-ish noun nearby, since alone it's too generic and false-positived on unrelated questions like "did you get a chance to look at the bugs list?"). A false negative here still falls back to the first fix's prompt-level guard; a false positive costs one extra, harmless lookup call.

Live-verified against the real deployed instance with a genuinely empty conversation history (no stale context available to lean on at all, ruling out prompt-following as the explanation): asked whether an Erika Odle email was ever forwarded to finance@ts.org — the forced first call used `search_email` (confirmed via inspecting the actual response's content blocks, not just the visible reply), and Charlie went on to run three broadening search rounds before answering "no," citing the two real forwards it found and correctly identifying them as unrelated BUG-023 test artifacts rather than assuming.

**Deliberately still open:** the forced-search hook only fires when the heuristic recognizes the question; the "don't trust an insufficiently broad negative" half of this bug is still prompt-level guidance, not a hard guarantee. Per this log's own standard, a scripted smoke test — even a strong one with empty history — is not the same as Jonathan noticing correct behavior (or catching a recurrence) across real day-to-day use over time.

**Touches:**
`core/agent.py` (system prompt, `search_email` tool description, `search_email`/`read_email_thread` dispatch handlers, `_looks_like_email_state_question()` + forced-tool_choice gate in `handle_turn()`)
